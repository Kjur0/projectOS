...
//		0	IC4			if set, the PIC espects to receive ICW4 during initialization
//		1	SNGL		if set, only 1 PIC in the system; if unset, the PIC is cascaded with slave PICs
//						and ICW3 must be send to controller
//		2	ADI			ignored on x86, set to 0
//		3	LTIM		if set, operate in level triggered mode; if unset, operate in edge triggered mode
//		4	INIT		set to 1 to initalize PIC
...
//		0	uPM			if set, operate in 80x86 mode; if cleared, in MCS-80/86 mode
//		1	AEOI		if set, on last interrupt acknowledge pulse, controller automatically performs
//						end of interrupt operation
//		2	M/S			only use if BUS is set; if set, selects buffer master; otherwise, selects buffer slave
//		3	BUF			if set, controller operates in buffered mode
//		4	SFNM	if set, controller operates in special fully nested modespecially fully nested mode; used in systems with large number of cascaded controllers
...
void i8259_Configure(uint8_t offsetPic1, uint8_t offsetPic2, bool autoEoi) {
	...
	// mask all interrupts until thay are enabled by the driver
	i8259_SetMask(0xFFFF);
}
...
bool i8259_Probe() {
	i8259_Disable();
	i8259_SetMask(0x2137);
	return i8259_GetMask() == 0x2137;
}
