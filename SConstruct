...
VARS.AddVariables(
    ...
    EnumVariable(
        "arch", help="Target architecture", default="i686", allowed_values=("i686")
    ),
    EnumVariable(
        "imageType",
        help="Type of image",
...
toolchainDir = Path(
    HOST_ENVIRONMENT["toolchain"], platform_prefix.removesuffix("-")
).resolve()
toolchainBin = Path(toolchainDir, "bin")
toolchainGccLibs = Path(
    toolchainDir, "lib", "gcc", platform_prefix.removesuffix("-"), DEPS["gcc"]
)
...
variantDir = "build/{0}_{1}".format(
    TARGET_ENVIRONMENT["arch"], TARGET_ENVIRONMENT["config"]
)
variantDirStage1 = variantDir + "/stage1_{0}".format(TARGET_ENVIRONMENT["imageFS"])

SConscript(
    "src/bootloader/stage1/SConscript", variant_dir=variantDirStage1, duplicate=0
)
SConscript(
    "src/bootloader/stage2/SConscript", variant_dir=variantDir + "/stage2", duplicate=0
)
